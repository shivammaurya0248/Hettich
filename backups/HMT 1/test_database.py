...
class DBHelper:
    ...
    def get_fl_status(self):
        ...
        if check is None:
            self.add_misc_data()
        self.c.execute('''SELECT fl_status FROM misc WHERE id=1''')
        try:
            data = self.c.fetchone()
            if data is not None:
                status = data[0]
                ...
                log.info(f"No fl_status found")
                return 0
        except Exception as e:
            log.error(f'ERROR: fetching fl_status misc data {e}')
            return 0

    def update_fl_status(self, status):
        try:
            self.c.execute('''SELECT * FROM misc''')
            check = self.c.fetchone()
            if check is None:
                self.add_misc_data()
            self.c.execute("UPDATE misc SET fl_status=?", (status,))
            self.conn.commit()
            log.info(f'Successful: FL_status updated successfully into database.')
        except Exception as e:
            log.error(f"Error: {e}")
    ...
    def add_misc_data(self):
        try:
            shift = get_shift(False)
            hour_ = datetime.now().strftime("%H")
            date_ = (datetime.now() - timedelta(hours=7, minutes=0)).strftime("%F")
            fl_status = 0
            self.c.execute("""INSERT INTO misc(id, current_shift, current_date_, current_hour,fl_status)
                                  VALUES (?,?,?,?,?)""",
                           (1, shift, date_, hour_, fl_status))
            self.conn.commit()
            log.info("Successful: Misc data added to the database.")
        except Exception as e:
            log.error(f'Error {e} Could not add Misc data to the Database')
        ...
        if shift == 'G':
            ...
            if data:
                if data[1] > data[0]:
                    ...
                    payload["G_cycle_time_parts"] = int(data[0] * 50)
                ...
                for t in data:
                    ...
                    if count == 1:
                        if t[0] is None:
                            ...
                            payload["A_cycle_time_parts"] = int(t[0] * 50)
                        else:
                            payload["A_cycle_time_parts"] = int(t[0] * 50)
                    ...
                    if count == 2:
                        if t[0] is None:
                            ...
                            payload["B_cycle_time_parts"] = int(t[0] * 50)
                        else:
                            payload["B_cycle_time_parts"] = int((t[0]) * 50)
                        # if t[1] is not None and t[0] is not None and t[1] > t[0]:
                        #     payload["B_cycle_time_parts"] = 0
                    if count == 3:
                        if t[0] is None:
                            ...
                            payload["C_cycle_time_parts"] = int(t[0] * 50)
                        else:
                            payload["C_cycle_time_parts"] = int((t[0]) * 50)
                        # if t[1] is not None and t[0] is not None and t[1] > t[0]:
                        #     payload["C_cycle_time_parts"] = 0
