...
class DBHelper:
    ...
    def add_sync_data(self, payload):
        try:
            ...
            # log.info('Successful Sync Payload Added to the database')
            # self.conn.commit()
            ...
            if data:
                self.c.execute("""UPDATE sync_data SET payload=?
                               WHERE date_=? AND shift = ? """,
                               (payload, payload['date_'], payload['shift']))

            else:
                self.c.execute("""
                                INSERT INTO sync_data(date_, shift,payload)
                                VALUES (?,?,?)""",
                               (payload['date_'], payload['shift'], json.dumps(payload)))
            self.conn.commit()
        except Exception as e:
    ...
    def delete_sync_data(self):
        try:
            ...
            self.conn.commit()
            log.info(f"Successful, Deleted from sync_data database")
        except Exception as e:
